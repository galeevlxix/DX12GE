...
---@param vec table Vector of type "vec3"
function PhysicsComponent:AddImpulseWithMagnitude(vec, matt)
    ...
    AddImpulse(self.object, vec[1], vec[2], vec[3], matt)
end
...
function PhysicsComponent:GetVelocity()
    ...
    assert_check(self.on == true, "GetVelocity: physics not active!")
...
--[[
The following functions are exported from C++ via sol2 and available in Lua:
| Lua Function Name       | Входные параметры (Lua)                              | Возвращаемое значение (Lua)                                 | Описание |
|-------------------------|------------------------------------------------------|-------------------------------------------------------------|----------|
| `GetObjectOnScene`      | `name: string`                                       | `Node3D*` (userdata) или `nil`                              | Возвращает указатель на узел сцены по его уникальному пути. |
| `GetTransformPosition`  | `object: Node3D*`                                    | `table { x = number, y = number, z = number }`              | Возвращает мировую позицию объекта как таблицу с координатами. |
| `TranslateTo`           | `object: Node3D*`, `x: number`, `y: number`, `z: number` | `1`                                                      | **ВАЖНО:** В текущей реализации устанавливает масштаб объекта (Scale=400) вместо перемещения. Требует исправления. |
| `TranslateBy`           | `object: Node3D*`, `x: number`, `y: number`, `z: number` | `1`                                                      | Смещает объект относительно текущей позиции. |
| `RotateBy`              | `object: Node3D*`, `yaw: number`, `pitch: number`, `roll: number` | `1`                                                   | Применяет относительное вращение (углы в радианах). |
| `GetVelocity`           | `object: Node3D*`                                    | `table { x = number, y = number, z = number }`              | Возвращает текущую линейную скорость физического объекта. |
| `AddImpulse`            | `object: Node3D*`, `x: number`, `y: number`, `z: number`, `massScale: number` | `1`                                         | Применяет мгновенный импульс к физическому объекту. |
| `GetNodeType`           | `object: Node3D*`                                    | `integer`                                                   | Возвращает числовой тип узла (`NodeTypeEnum`). |
| `GetWorldDirection`     | `object: Node3D*`                                    | `table { x = number, y = number, x = number }`              | **ОШИБКА:** Возвращает некорректную таблицу (дублирует "x" вместо "z"). Возвращает направление "вперёд" объекта в мировых координатах. |
| `GetChild`              | `object: Node3D*`, `childName: string`               | `Node3D*` (userdata) или `nil`                              | Возвращает дочерний узел по имени. |
| `GetParent`             | `object: Node3D*`                                    | `Node3D*` (userdata) или `nil`                              | Возвращает родительский узел. |
| `CallError`             | `text: string`                                       | `1`                                                         | Выводит сообщение об ошибке в консоль C++. |
| `Register`              | `className: string`                                  | `1`                                                         | Регистрирует имя класса в списке активных Lua-классов с проверкой на дубликаты. |
| `LoadObjectWithModel`   | `name: string` — имя/путь модели                     | `1`                                                         | Загружает 3D-модель на сцену и автоматически создаёт Lua-объект с компонентами Transform и Physics. |
| `GetClass`              | `name: string`                                       | `table` (класс Lua) или `nil`                               | Возвращает таблицу класса по имени. |
| `GetComponent`          | `object: Node3D*`, `component: string`               | `table` или `nil`                                           | Возвращает компонент или поле Lua-скрипта, привязанного к узлу. Ищет по шаблону "className.componentName". |
| `GetAIState`            | `object: AINode*`                                    | `boolean`                                                   | Возвращает состояние ИИ (`true` — активен, `false` — выключен). Обрабатывает исключения. |
| `SetAIState`            | `object: AINode*`, `state: boolean`                  | `1`                                                         | Включает/выключает ИИ-поведение у узла. Обрабатывает исключения. |
| `GetCurrentCamera`      | отсутствуют                                          | `CameraNode*` (userdata)                                    | Возвращает указатель на текущую активную камеру. |
| `CastRay`               | `distance: number`                                   | `string` (путь к объекту) или `"empty "`                    | Выполняет рейкаст от текущей камеры в направлении взгляда на указанную дистанцию. Возвращает путь к ближайшему объекту. |
| `ChangeCamera`          | отсутствуют                                          | `Node3D*` (userdata)                                        | Переключает камеру между первым лицом ("root/fp_player") и третьим лицом ("root/tp_player"). **ВАЖНО:** Всегда возвращает tp_player. |
| `DestroyNodeByNode`     | `object: Node3D*`                                    | `1`                                                         | Уничтожает узел и автоматически очищает все связанные с ним Lua-классы из системы. |
| `DestroyNodeByNodePath` | `path: string`                                       | `1`                                                         | Уничтожает узел по пути без очистки Lua-классов (требует ручной очистки). |
| `SetObjectVelocity`     | `object: Node3D*`, `x: number`, `y: number`, `z: number` | `1`                                                      | Устанавливает линейную скорость физического объекта. |
