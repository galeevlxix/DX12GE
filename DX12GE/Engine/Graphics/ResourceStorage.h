...
#include "../Base/AudioWavComponent.h"
...
public:
	...
	static uint32_t AddAudio(const std::string& name);
	...
	static std::shared_ptr<AudioWavComponent> GetAudio(uint32_t id);
	...
	static std::shared_ptr<AudioWavComponent> GetAudioByName(const std::string& name);

	static void DeleteObject3DComponentForever(uint32_t id);
	static void DeleteTextureComponentForever(uint32_t id);
	static void DeleteAudioComponentForever(uint32_t id);
