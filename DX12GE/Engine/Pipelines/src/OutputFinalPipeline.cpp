#include "../OutputFinalPipeline.h"

void OutputFinalPipeline::Initialize(ComPtr<ID3D12Device2> device)
{
	CreateRootSignature(device);
	CreatePipelineState(device);

	PipelineState.Get()->SetName(L"Output Final Pipeline State");
	RootSignature.Get()->SetName(L"Output Final Root Signature");
}

void OutputFinalPipeline::Set(ComPtr<ID3D12GraphicsCommandList2> commandList)
{
	commandList->SetPipelineState(PipelineState.Get());
	commandList->SetGraphicsRootSignature(RootSignature.Get());
}

void OutputFinalPipeline::CreateRootSignature(ComPtr<ID3D12Device2> device)
{
	ThrowIfFailed(
		D3DReadFileToBlob(L"OutputFinalVertexShader.cso", &m_VertexShaderBlob));
	ThrowIfFailed(
		D3DReadFileToBlob(L"OutputFinalFragmentShader.cso", &m_PixelShaderBlob));
...
void OutputFinalPipeline::CreatePipelineState(ComPtr<ID3D12Device2> device)
{
