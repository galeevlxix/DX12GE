...
Для сериализации и десериализации сцены был выбран формат JSON, так как он наиболее читабельный и простой для реализации.
Для работы с JSON я использую библиотеку `nlohmann/json.hpp`. Новый класс будет иметь два метода для сохранения и загрузки сцены, а также путь
к самому файлу сохранения. 
```c++
class JsonScene
{
private:
...
```
Чтобы сохранить состояние сцены, нужно сохранить информацию об компоненте объекта, а также его позицию, поворот и масштаб.
```c++
#include "../JsonScene.h"
#include "../json.hpp"
